...
class EyeQ_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        """
        # Uncomment this section to run in the supervised mode
        suf = random.choice(['001', '010', '011', '100', '101','110', '111'])
        # PQ_file = os.path.splitext(self.PQ_images[generate_HQ_index])[0] + '_' + suf + '.jpeg' 
        PQ_file = HQ_file[:-4] + '_' + suf + '.jpeg'
        # PQ_file = os.path.splitext(self.PQ_images[idx])[0] + '.jpeg'
        """
        PQ_file = random.choice(os.listdir('/scratch/vvasa1/GSL research/unsupervised data/train/LQ'))
